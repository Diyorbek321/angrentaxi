 ...
 * cannot tell a valid token from a forged or expired one. Anyone can hand-write
 * an `access_token` cookie and get past this. Its only job is to stop a logged-out
 * browser from rendering panel screens before the client-side redirect kicks in.
 * Every request that actually returns data is still authorized by the backend,
 * and that remains the real boundary.
 */

/** Segments the panel serves only to a signed-in user. */
