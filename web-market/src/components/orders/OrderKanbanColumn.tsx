...
export function OrderKanbanColumn({ status, count, children }: OrderKanbanColumnProps) {
  ...
  return (
    <section
      className="w-72 shrink-0 flex flex-col min-h-0 rounded-xl bg-surface-2/60 border border-line"
    ...
    >
      <header className="shrink-0 flex items-center gap-2 px-3 h-11 border-b border-line">
        ...
        <h2 className="text-sm font-semibold text-ink truncate">{meta.label}</h2>
        <span className="ml-auto min-w-6 h-5 px-1.5 rounded-full bg-surface border border-line text-2xs font-mono font-semibold text-muted flex items-center justify-center tabular-nums">
          {count}
        </span>
      </header>
